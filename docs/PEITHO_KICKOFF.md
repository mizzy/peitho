...
- `peitho:shadow-mounted` — payload `{ root, key, index }`, where `root` is the slide's `ShadowRoot` (present, preview) or its `<section class="peitho-slide">` element (distribution viewer, PDF, lint). Fires once per mounted slide, and each payload is also pushed onto `window.__peithoShadowRoots`, so a script that loads after the mount (a module, or an inline script behind a blocking `<script src>`) drains what it missed. Two things set it apart from the entries above: it is the first notification that carries a **DOM handle**, because a layout `<script>` cannot reach its own slide any other way (`document.querySelectorAll` and `MutationObserver` never cross a shadow boundary, and `document.currentScript` is `null` inside a shadow root); and it fires in **every output mode**, including PDF export and lint where no shell exists — which is what keeps the second invariant below true rather than violated: a layout script written against this event works whether or not a shell is present. It grants no way to request anything; a slide that listens to it still may not issue `navigate`. Design record: `docs/plans/2026-09-23-layout-scripts.md`.
