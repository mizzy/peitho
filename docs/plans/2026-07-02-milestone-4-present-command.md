...
Across all 15 tasks, we first add `tiny_http` and the volatile cache ignore, making it possible to generate `notes_json` and `present.html` from the Rust core. Next we add `installSyncBridge` on the TS side and test the send/receive of sync messages and the echo stop via no-op. After that we carve out the CLI's build pipeline to be reusable, so that `peitho present` recreates `.peitho/present-cache/` every time and emits slides/manifest/CSS/notes/present.html/shell.js. Finally, we complete the volatile presentation entry point by going through the tiny_http server, path-traversal prevention, URL display, best-effort browser launch, a repository smoke test using the real shell, and the pre-Rust-test shell build in CI.
