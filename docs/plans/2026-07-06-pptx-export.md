...
MeasuredParagraph { align, bulletLevel: Option<u8>, runs: Vec<MeasuredRun> }
...
- リストは`<li>`→`bulletLevel`(ネスト深さ)。見出し・段落はcomputed font-size/weightがrunに乗る
...
- 画像: `<p:pic>`、rect位置、`ppt/media/`にコピーしrelで参照。altは`<p:nvPicPr>`のdescr
